...
The specific preparation for each material is unique and can be found in the Test Items.
...
Most commercial fingerprint scanners are designed to output 500 dpi grayscale images, however there are some that produce 1000 dip images. The native hardware resolution of the scanner is oftentimes higher than the output resolution. This higher resolution is then reduced in software to 500 dpi. If 1000 dpi resolution scanners are not readily available, the evaluator may need to purchase a specialized version of the scanner or scanner driver software that outputs the preferred 1000 dpi images. Some scanner manufactures may be willing to provide the special driver at nominal cost. Alternatively, the evaluator could use a Software Developer’s Kit (SDK) to develop his own driver. Many fingerprint scanner manufactures provide free SDK’s for their devices.

...
A fingerprint mold is the template that is created from the fingerprint that will be attacked. There are many ways to create a mold, such as directly from the finger, or from a scanned fingerprint image that is printed on a PCB. A mold should be able to be reused repeatedly for the purposes of this testing.

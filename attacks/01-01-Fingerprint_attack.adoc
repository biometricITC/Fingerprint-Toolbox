...
Attack Potential values are shown in <<calculatedtable1-1>> (all variants have the same Attach Potential Calculations). Attack Potential values for Identification account for the time, expertise, etc. required to make the mold and the cast described in this attack. When selecting the mold / cast combination, consideration must be given to the ability to produce the mold separately from that needed for the cast. 

For example, the resulting attack potential for Identification in <<calculatedtable1-1>> is computed by combining the mold (<<moldtable1-1>>) and the cast (<<casttable1-1>>) values per-Factor, as follows:

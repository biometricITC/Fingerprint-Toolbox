 ...
 ** For example, an Elapsed Time for the mold of <= three days and for the cast of <= one day when added results in a total of <= four days, which is assigned the Identification Value of <= one week. 
...
It may be possible for some casts to be completed within an hour utilizing this mold, but many of the cast materials require drying time before they can be used. For simplicity, the maximum value for exploitation is used for all attacks.

*All Variations*

.Calculated Attack Potential Fingerprint attack FP1-01-xx
[[calculatedtable]]
...
6.+^.^|Calculated Total Attack Potential = 9 < Basic Attack Potential

|===

.Mold Attack Potential Fingerprint attack FP1-01-xx
[[moldtable]]
...
|*Elapsed Time*
|<= two days (a proportion of the total of less than three days)
|1
...
3.+^.^|Mold-only Total Attack Potential for Identification = 1

|===

.Cast Attack Potential Fingerprint attack FP1-01-xx
[[casttable]]
[cols=".^2,.^2,^.^1",options="header",]
|===
|Factor 
...
3.+^.^|Cast-only Total Attack Potential for Identification = 1
